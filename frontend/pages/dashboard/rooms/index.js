...
export default {
  async asyncData(context) {
    ...
    let response = await context.$axios.$get("/admin/rooms?is_paginated=1&type=focus").catch((e) => {
      console.log('err: ', e)
    })
  ...
  data() {
    return {
      titlePage: this.$t('admin.focus_rooms'),
      orderBy: 'id',
      orderType: 'DESC',
      fieldsData: [{
          key: "id",
  ...
  methods: {
    ...
    async loadAsyncData() {
      ...
      this.queryParam = `?page=${this.meta.current_page}&is_paginated=1&type=focus&public_search=${this.publicSearch}&orderBy=${this.orderBy}&orderType=${this.orderType}`

      await RoomService.getAll(this.queryParam)
        .then((response) => {
  ...
  computed: {
    titleStack() {
      return [this.$t('admin.focus_rooms')]
    }
  },
  head() {
    return {
