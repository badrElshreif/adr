...
export default {
  ...
  methods: {
    ...
    async handleUploadFile (e, type = 'image') {
      ...
      if (e.target.files.length) {
        ...
        if(! imageExt.includes(extension) && type == 'image'){
          ...
          this.form.icon = '';
          return false;
        } else if(! videoExt.includes(extension) && type == 'audio'){
    ...
    async update() {
      await SoundService.update(this.form, this.param_id)
        .then(() => {
          this.back()
    ...
    async create() {
      ...
      await SoundService.create(this.form)
        .then(() => {
          this.back()
    ...
    back() {
         return this.$router.push(this.localePath({name: "dashboard-sounds"}))
    }
  },
}
