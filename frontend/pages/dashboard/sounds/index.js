...
export default {
  async asyncData(context) {
    //let response = await SoundService.getAll()
    let response = await context.$axios.$get("/admin/files?is_paginated=1&type=sound").catch((e) => {
      console.log('err: ', e)
    })
  ...
  methods: {
    ...
    async loadAsyncData() {
      ...
      this.queryParam = `?page=${this.meta.current_page}&is_paginated=1&type=sound&public_search=${this.publicSearch}&orderBy=${this.orderBy}&orderType=${this.orderType}`

      await SoundService.getAll(this.queryParam)
        .then((response) => {
